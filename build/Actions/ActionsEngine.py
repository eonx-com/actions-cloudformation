...
from Actions.ActionsTemplate import ActionsTemplate
...
class ActionsEngine:
    ...
    @staticmethod
    def generate(environment_name, environment_path, template_path):
        """
        Generate CloudFormation Template

        :type environment_name: str
        :param environment_name: The name of the environment for use in templates (e.g. 'dev', 'uat', 'prod')

        :type environment_path: str
        :param environment_path: The base path under which the configuration files are located
        ...
        :return:
        """
        ActionsEngine.environment_path = environment_path
        ActionsEngine.template_path = template_path
                        ...
                        if key == 'resources':
                            ...
                            for resource_id, resource in value.items():
                                if 'template' not in resource:
                                    ...
                                    exit(1)
                                yaml_resource_templates[resource_id] = ActionsEngine.load_template_file(
                                    resource['template'])
        ...
        return ActionsEngine.create_cloudformation_template(
    ...
    def create_cloudformation_template(stack_config, resource_configs, resource_templates) -> str:
        ...
        """
        output_file = ''
        output_file += ActionsEngine.create_file_header(stack_config)
        output_file += ActionsEngine.create_file_resources(stack_config, resource_configs, resource_templates)
    ...
    def create_file_header(stack_config) -> str:
        ...
        if 'description' in stack_config:
            ...
            config_description = 'CloudFormation template generated by Actions'
    ...
    def load_template_file(filename) -> str:
        ...
        filename = ActionsEngine.template_path + '/' + filename.lstrip('/')
    ...
    def print_dictionary(dictionary) -> None:
        """
        Pretty-print a dictionary as a series of `key = value` pairs
    ...
    @staticmethod
    def dictionary_to_dotted(dictionary, parent_id=None):
        """
        Return a new dictionary containing all keys from the supplied dictionary in dot notation

        :type dictionary: dict
        :param dictionary: The dictionary to convert to dot notation

        :type parent_id: str
        :param parent_id: Parent key (used during its internal iterator- leave as None when calling unless you want a prefix added to all names)
        ...
        for key, value in dictionary.items():
            if parent_id is None:
                ...
                current_id = '{parent_id}.{key}'.format(parent_id=parent_id, key=key)

            if isinstance(value, dict) is True:
                result.update(ActionsEngine.dictionary_to_dotted(value, current_id))
            else:
